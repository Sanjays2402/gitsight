...
test('defaultDiffSettings is scroll + show-whitespace', () => {
  assert.deepEqual(defaultDiffSettings(), { wrap: false, ignoreWhitespace: false });
});
...
test('wsParam maps to the git -w request only when ignoring', () => {
  assert.equal(wsParam({ wrap: false, ignoreWhitespace: true }), 'ignore');
  assert.equal(wsParam({ wrap: false, ignoreWhitespace: false }), undefined);
});
...
test('coerceDiffSettings reads strict booleans', () => {
  assert.deepEqual(coerceDiffSettings({ wrap: true, ignoreWhitespace: true }), {
    wrap: true,
    ignoreWhitespace: true,
  });
  // Truthy-but-not-true values are not accepted.
  assert.deepEqual(coerceDiffSettings({ wrap: 1, ignoreWhitespace: 'yes' }), {
    wrap: false,
    ignoreWhitespace: false,
  });
  // Missing keys default to false.
  assert.deepEqual(coerceDiffSettings({ wrap: true }), { wrap: true, ignoreWhitespace: false });
});

test('diffSettingsEqual compares by value', () => {
  assert.equal(diffSettingsEqual({ wrap: true, ignoreWhitespace: false }, { wrap: true, ignoreWhitespace: false }), true);
  assert.equal(diffSettingsEqual({ wrap: true, ignoreWhitespace: false }, { wrap: false, ignoreWhitespace: false }), false);
});
