...
### Roadmap refill (W48-W53, added tick 36 — W43-W47 all shipped. Six-view app is deep; these add NEW surfaces + polish. Keyboard-first, local-first, no new deps.)
- [ ] **W48** Activity year deep-link + keyboard — extend W24 hashRoute with `activity?year=YYYY&metric=churn` so a scoped calendar is shareable, and add left/right-arrow year stepping when the Activity tab is focused. Pure hash grammar (reuse W43 `isCalendarYear`) + a small keyboard handler.
- [ ] **W49** Minimap commit hover/jump (W45 deepening) — hovering the minimap shows a tooltip with the nearest commit's subject+sha, and a click selects that row's detail (not just scrolls). Pure nearest-mark lookup (`markAtY`) + reuse the GraphController selection.
- [ ] **W50** File-tree filter in the commit detail — when a commit touches many files, add a path-substring filter box above the file list (detail panel + compare both). Pure `filterFileChanges(files, query)` (case-insensitive path match) + a small input; reuse across both surfaces.
- [ ] **W51** Blame author email link to contributor — clicking a blame author dot/name opens that author's W23 detail panel (currently only the legend isolates). Pure author->email resolution from the blame model + reuse `authorPanel.open`.
- [ ] **W52** Compare as patch — a "Copy as patch" button on the compare view that assembles the unified diff of the changed files into a clipboard-ready `git format-patch`-style blob (header + per-file hunks). Pure patch assembler over the already-fetched FileDiffs (+tests); no new endpoint.
