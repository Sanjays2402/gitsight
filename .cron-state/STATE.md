...
### Tick 18 candidates (drafted now so future ticks don't restart cold)
...
- [ ] F82: Per-commit benchmark scorer — runs a configured benchmark command per commit in `<upstream>..HEAD` and charts the result. Extends F55 with quantitative output.
- [ ] F83: Commit graph PNG export — companion to F61 SVG export.
- [ ] F103: PR template lint diagnostics — warn about empty checkboxes / unfilled TODOs at PR-create time. Composes with F98 (template parser is reusable).
- [ ] F104: "Open issue from selection" — convert TODO/FIXME comments into a `gh issue create` draft. Composes with F99 (issue picker, in reverse).
- [ ] F105: Inactive-reviewer detection — when a review-requested PR has a reviewer who's been silent N days, surface them in F75 with a "remind via @-mention" action.
- [ ] F106: PR ready-for-review timeline — for the current branch's open PR, render a status row (commits since last review / last comment / requested reviewers / mergeability) in the status bar.
- [ ] F107: Conflict resolution coach extension — when stagedConflictGate fires (F78), surface a per-conflict context picker that shows the base + ours + theirs hunks side-by-side via a 3-pane diff editor.
