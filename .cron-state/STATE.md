...
- [ ] **W31** Diff view settings — per-file unified/split toggle + whitespace-ignore + wrap, persisted; the diff renderer (W7) is unified-only today. Pure render-option plumbing through `diffView`/`renderFileDiff`.
...
- [ ] **W32** Command palette deepening — recent commits + a sha/grep typeahead inside Cmd-K (jump straight to a commit's detail), plus the W30 saved filters as palette items. Reuse `paletteSearch` ranking; feed it the snapshot's commits.
- [ ] **W33** Activity calendar range/heat controls — toggle the metric (commits vs churn via a cheap `/api/activity` extension), pick a year, and a streak readout (current/longest run of active days). Pure streak + bucketing in `src/shared/activity.ts`.
- [ ] **W34** Diff word-level intra-line highlighting — within a changed line pair, mark the changed spans (a pure LCS/diff over the two strings) so a one-char edit doesn't read as a whole-line change. Pure `tokenDiff` module + render in `diffView`.
...
- [ ] **W37** Keyboard help overlay (?) — a discoverable cheat-sheet of every shortcut (/, j/k, Enter, Cmd-K, Esc), grouped by view, in the app's modal language; pairs with the now keyboard-first UX.
