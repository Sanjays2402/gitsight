...
- [ ] **W26** Command palette / quick-switcher — a Cmd-K overlay to jump to any view, ref, or recent search (fuzzy match), so the app is keyboard-first like Linear/Raycast. Pure fuzzy-rank + a modal; no backend.
- [ ] **W27** Per-commit permalink — `#commit/<sha>` deep link (extend `hashRoute`) that opens the detail panel on load, so a specific commit is shareable the way a comparison is (W24).
- [ ] **W28** Graph row context menu — right-click a commit for Copy SHA / Copy message / Compare from here / Blame at this commit / Open on remote (host-detect via the existing `hostDetect`-style URL builder), instead of only the detail-panel actions.
- [ ] **W29** Branch/tag detail in the ref rail — click a ref's caret for a popover with its tip commit, ahead/behind vs HEAD (reuse the W18 compare math), and last-activity, without leaving the graph.
- [ ] **W30** Search history + saved filters — persist recent structured queries (W10) to localStorage, surface them under the search box, and let a query be pinned; a "filters" affordance that doesn't exist yet.
