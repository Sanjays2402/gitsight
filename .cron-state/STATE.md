...
- [ ] F93: PR comment composer — pair with F88. Compose-and-post a comment from the editor's current selection (`gh pr comment <num> --body-file -`). Inline review comments would need the gh API directly since `gh pr review` only accepts single body+state.
- [ ] F94: Repository-level secret audit summary view — F89 sister command that shows ALL repos in the current workspace with their secret-audit status as a single tree.
- [ ] F95: Auto-split commit suggester (F90 extension) — given a `huge` diff, run a coherence classifier (file path + first-line subject + churn cluster) to suggest a per-commit split plan with `git add -p`-ready hunk ranges.
- [ ] F96: Reviewer self-review heuristic (F91 extension) — when the PR author IS the dominant shortlog contributor and there are no other candidates, suggest the F47 "files I own" picker explicitly so the user knows the picker drained itself for a legitimate reason.
- [ ] F97: Release auto-publish-on-merge — F92 extension. When the merged PR's commit body matches a `Release-as: v1.2.3` trailer, skip the prompt entirely and run the release chain headlessly.
