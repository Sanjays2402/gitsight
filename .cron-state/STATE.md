...
### Tick 20 candidates (drafted now so future ticks don't restart cold)
...
- [ ] F112: Review-comment AI summary (when >10 comments, AI summarises discussion into a paragraph + open-questions list).
- [ ] F114: Lazy-load complexity score for `gh pr view` — show the F111 badge buckets aggregated for a PR's changed files in the prTimelinePill tooltip.
- [ ] F115: gh-aware merge queue surface — show GitHub merge queue position + estimated minutes for the current PR.
- [ ] F116: Pre-commit signed-off-by enforcement — when DCO is required (CONTRIBUTING.md mentions DCO / Signed-off-by), add a SCM warning if the about-to-commit message is missing the trailer.
- [ ] F117: "Last release vs HEAD" diff size + CHANGELOG suggestion preview — composes with F92 tag-on-merge.
