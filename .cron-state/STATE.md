...
- [ ] F98: gh PR template integration — when composing a PR description (F2 / F77 / F87), pre-fill the body from `.github/PULL_REQUEST_TEMPLATE.md` and offer per-section AI fill-in.
- [ ] F99: GitHub issue link inserter — quick-pick over open issues, insert `#NN` reference at cursor or append `Closes #NN` trailer to the SCM input box (composes with F73 commit footer).
- [ ] F100: Repo-level "what's mine?" dashboard — for the current author, list open PRs / unmerged commits / open issues / pending reviews as a single tree (composes F75 + F88 + per-author commit stats).
- [ ] F101: Auto-pr-checkout for selected review — F75 follow-up: when the user picks an "awaiting your review" PR, offer `gh pr checkout` with the same-repo guard from F75 turned into a pre-flight.
