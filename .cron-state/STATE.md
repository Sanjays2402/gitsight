...
- [ ] **W21** Virtualised blame for huge files (the blame view soft-caps at 4000 lines today; window it).
- [ ] **W22** Activity drill-down — a per-day commit list panel when a calendar cell is opened (instead of only routing to the graph filter).
- [ ] **W23** Contributor detail — click-through to a per-author mini-dashboard (their files, their activity sparkline).
- [ ] **W24** Compare view deep-link — ref-pair in the URL hash so a comparison is shareable/bookmarkable; also a "compare from here" action on a graph row.
- [ ] **W25** Stash apply/pop/drop actions — the web stash view is read-only today; a guarded local-only mutate path (companion POST + confirm) would close the gap with the extension's stashVisualizer.
