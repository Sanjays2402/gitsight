...
- [ ] **W35** Contributor compare — select two authors in the leaderboard and see their commit/churn/file-overlap side by side (reuse W23 author detail + a pure overlap calc). A "vs" affordance the panel lacks.
...
- [ ] **W38** Diff split view — the deferred half of W31: a two-column (old | new) renderer toggle in the diff settings, reusing the parsed FileDiff + the W34 word segments. Pure column-pairing helper + a `renderFileDiffSplit` in `diffView`.
- [ ] **W39** Activity metric toggle — the deferred half of W33: switch the calendar between commits and churn (a cheap `/api/activity` numstat extension), plus a year picker. Pure churn-bucketing in `src/shared/activity.ts`.
- [ ] **W40** Blame age legend + author filter — the blame heatmap shows colour but no key; add an age-ramp legend + click an author in the legend to dim the others' lines. Pure legend-bucket helper, reuse the W12 blame model.
...
- [ ] **W42** Stash create from the web app — a guarded POST (mirrors W25 apply/pop/drop) to `git stash push` with an optional message + include-untracked, behind `--allow-mutations`. Pure argv builder + a small form.
