...
- `web/server/index.mjs` — companion (parseArgs +`--root`, buildSnapshotForRepo, buildCommitDetailForRepo, buildFileDiffForRepo, scanReposUnder, resolveRequestRepo, isSafeRev, createCompanionServer). Routes: /api/{health,repos,graph,commit/:rev,diff}. Tests: `web/server/index.test.mjs`.
- `web/src/{main,graph,data,theme,themeResolve,palettePicker,format,icons,demo}.ts`, `styles.css`.
- `web/src/{detailPanel,detailFormat}.ts` (W6), `{diffView,diffFormat}.ts` (W7), `repoPicker.ts` (W8), `refRailView.ts` (W9). Tests: `web/src/{themeResolve,detailFormat,diffFormat}.test.mjs`.
