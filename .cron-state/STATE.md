...
### Roadmap refill (W60-W65, added tick 38 — W54/W55/W57/W59 shipped; W56+W58 deferred-with-reason above. Keep deepening the six-view app, keyboard-first + local-first, no new deps.)
- [ ] **W60** Contributor churn aggregate + sort (the real W56) — add a server-side numstat fold so `/api/contributors` carries per-author insertions/deletions, then a commits/churn/recency sort toggle in the leaderboard header. Shared `buildContributors` gains optional churn fields; a new companion read (`git log --numstat --author` per author is too many reads — instead one `git log --numstat` pass folded by email). Pure `sortContributors(stats,key)` comparator (+tests) + a segmented control. This is the proper version of the deferred W56.
- [ ] **W61** Activity peek keyboard a11y — the W55 popover is hover-only; surface the same top-subjects preview on cell focus (keyboard nav) so it's not mouse-exclusive. Reuse `DayPeekController` (add focus/blur alongside enter/leave) + position relative to the focused cell. Small slice; no new pure logic, just an a11y deepening.
- [ ] **W62** Compare commit filter -> jump — the W54 filter narrows the columns; let Enter in the filter box open the first matching commit's detail (reuse `onOpenCommit`) so a search resolves to an action, not just a narrowed list. Tiny wiring slice + a "first match" accessor.
- [ ] **W63** Stash filter deep-link — extend W24 hashRoute with `stashes?q=<filter>` so a filtered stash view is shareable; opening the link pre-fills the box + narrows. Mirrors the W48/W57 param grammar; pure `sanitizeStashQuery` (bounded, control-char-stripped) + build/parse + a small `pendingStashQuery` boot stash.
- [ ] **W64** Graph density toggle (the deferred W58) — parameterise the shared row height across `graphCore`/`graph.ts`/`virtual.ts`/minimap so a comfortable/compact switch (persisted like the theme) re-packs long histories. Pure density->row-height map (+tests) threaded through `computeWindow` + the lane-SVG geometry. The careful multi-file version W58 punted.
- [ ] **W65** Blame "copy line range" — shift-click a second line number to select a range and copy a `#blame?...&line=N-M` multi-line permalink (extend the W57 grammar to accept a `N-M` range + reveal the start). Pure range parse/format in hashRoute + a small range-selection state in the blame view.
