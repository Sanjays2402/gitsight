...
### Tick 17 candidates (drafted now so future ticks don't restart cold)
...
- [x] F98: gh PR template integration — DONE tick 18 — see above.
- [x] F99: GitHub issue link inserter — DONE tick 18 — see above.
- [x] F100: Repo-level "what's mine?" dashboard — DONE tick 18 — see above.
- [x] F101: Auto-pr-checkout for selected review — DONE tick 18 (shipped as full pre-flight with 5 checks rather than naive checkout - more useful surface).
- [x] F102: CODEOWNERS validator — DONE tick 18 — see above.
...
- 2026-06-22 16:34 PT — 5 features shipped: F103 `9d9b327`, F104 `a59f329`, F105 `f45e4cc`, F106 `71c0fef`, F107 `47c5112`. Gate: lint ok, compile ok, 1545/1545 tests green (1381 → 1545, +164 new). New configs: 9 (prTemplateLint.{enabled,flagEmptyCheckboxes,requiredSections} = 3, inactiveReviewers.{staleAfterDays,includeTeams,includeCommented} = 3, prTimelinePill.{enabled,refreshSeconds,hideWhenReady} = 3). New commands: 5 (lintPrTemplate, openIssueFromSelection, findInactiveReviewers, prTimelinePill.refresh, conflictCoach). New diagnostic collections: 1 (gitsight.prTemplate on PR-body markdown buffers). New status-bar pills: 1 (PrTimelinePill at priority 86 — leftmost of the gh-driven stack: 86 prTimeline, 87 diffSize, 88 secretAudit, 89 stagedConflict, 90 actionsPill, 91 forgottenFiles, 92 submodulePill). New providers: 1 CodeActionProvider (Refactor-kind for "Open issue from selection" on non-empty selections inside git repos) + 1 TextDocumentContentProvider (`gitsight-conflict:` scheme for the 3-way diff coach). New pure helpers: 5 (prTemplateLint, issueFromSelection, inactiveReviewers, prTimeline, conflictCoach). New view-layer files: 5 (prTemplateLint, openIssueFromSelection, inactiveReviewers, prTimelinePill, conflictCoach). New tests: 164 (F103: 30 placeholder regexes + fence-aware checkbox suppression + multi-line HTML comments + untouched-section + missing-required + severity aggregation, F104: 42 comment-stripping across 6 syntaxes + marker word-boundary + multi-line body extraction + title truncation + label mapping + draft composition + code-fence escape + language fence normalisation, F105: 34 parsing + normalisation (team vs user) + latest-review wins + days clamping + draft-PR skip + team-skip toggle + tone selection + mention composition + singular/plural days + summary truncation, F106: 31 parsing across gh shape variants + latest review wins + commits/comments since review + full state decision tree + glyph completeness + tooltip rendering, F107: 27 2-way + diff3 extraction + ref-name capture + malformed-block detection + all four resolution choices + multi-block targeting + range error + all 5 difficulty cases for trivial + line-count thresholds). Notable patterns added: (1) PR-body lint that detects untouched-from-template sections by parsing both bodies and byte-comparing per-section content (composes with F98 prTemplate parser); (2) comment-stripping classifier across //, #, /*, --, ;, %, <!-- delimiters with marker (TODO/FIXME/XXX/HACK/NOTE/OPTIMIZE/REVIEW/BUG) extraction and scope-paren drop; (3) latest-review-wins normalisation that prefers APPROVED-after-CHANGES_REQUESTED over chronological ordering (matches GitHub's review-decision semantics); (4) commits-since-last-review computation that gates on hasPriorReview so a brand-new PR without any reviews doesn't get a misleading "+N since review" pill; (5) virtual-document content provider that caches per-block payloads under encoded URIs, letting `vscode.diff` render 3-way previews without writing temp files; (6) per-block difficulty heuristic with auto-suggestion (5 trivial cases including ours-matches-base + theirs-matches-base in diff3 mode); (7) round-robin tone picker pattern (gentle / firm / custom prefix) for reminder composition - first use of a three-way tone selector in gitsight, can be reused for footer / commit composer prompts later.
