...
### Tick 22 candidates (drafted now so future ticks don't restart cold)
...
- [ ] F124: Reviewer load-balancer (composes with F85+F91+F121); rolling PR-throughput per handle, suggests reviewers whose queue clears fastest.
- [ ] F125: Test-impact -> PR template comment (composes with F122+F98); auto-injects "Likely-touched tests" section into the PR body on first push.
- [ ] F126: Branch protection rule auto-suggester (composes with F119); for unprotected default branches, suggest a sensible starter ruleset (PR review + status checks).
- [ ] F127: Stale stash auto-export (composes with F120); for drop-safe stashes, offer to write the diff to a timestamped patch file before dropping.
