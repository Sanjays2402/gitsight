...
- [ ] **W53** Stash diff split view — carry the W38/W46 split toggle into the stash file diffs (currently unified only); reuse `diffSettingsStore.layoutFor('stash')` as a third surface. Small wiring slice + a Split pill in the stash card header.

### Roadmap refill (W54-W59, added tick 37 — W48-W52 shipped; W53 still open. Keep deepening the six-view app, keyboard-first + local-first, no new deps.)
- [ ] **W54** Compare commit-list filter — the compare view's ahead/behind columns can get long; add a small subject/author filter box above them (reuse a `matchCommits`-style pure matcher) so you can find a specific commit in a wide range. Pure matcher + a thin input per column or one shared box.
- [ ] **W55** Activity tooltip → mini day summary — hovering an activity cell currently shows a bare title; replace it with a small custom popover listing the top 1-2 commit subjects for that day (reuse the W22 /api/day fetch, debounced + cached). Pure popover-position helper + a hover controller.
- [ ] **W56** Contributor sort options — the leaderboard is commit-count-sorted only; add a sort toggle (commits / churn / recency) with a pure `sortContributors(stats, key)` comparator (+tests) and a segmented control in the contributors header.
- [ ] **W57** Blame line permalink — clicking a blame line's number copies a `#blame?path=&rev=&line=N`-style deep link (extend W24 hashRoute with a blame grammar + sanitisers) so a specific blamed line is shareable; opening the link loads the file + jumps to the line (reuse the W21 revealLine path).
- [ ] **W58** Graph density toggle — a comfortable/compact row-height switch for the commit graph (persisted like the theme), so long histories can pack tighter on demand. Pure density->row-height map + a control; thread the height through the W16 windowing math.
- [ ] **W59** Stash search/filter — the stash list can grow; add a message/branch filter box above the cards (pure substring matcher over the parsed stash list) so a specific WIP is findable. Small wiring slice + reuse the fileFilter CSS pattern.
