...
### Tick 22 candidates (RESOLVED above)
...
### Tick 23 candidates (drafted now so future ticks don't restart cold)
...
- [ ] F118: PR description from active diff selection — extends F87 with multi-file gather.
- [ ] F128: Reviewer load-balancer integration with F57 picker — wire the F124 re-rank into showDefaultReviewersPicker as a third composition step after F85 round-robin + F91 shortlog. Today F124 is a standalone report; F128 makes the picker order it produces actually reflect the load profile.
- [ ] F129: Test-impact PR body auto-sync — fire-and-forget hook into gitsight.push that calls injectTestImpactIntoPr automatically when a `Likely-touched tests` section is already in the PR body. Mirrors F77 PR-draft sync's auto-refresh behaviour.
- [ ] F130: Branch protection apply-on-create — when `gitsight.createBranch` lands a new release/* branch, automatically offer the F126 suggestion picker as a follow-up. Composes with F110 branchNamer.
- [ ] F131: Stash patch import — companion to F127; pick a .patch file and apply it via `git apply --3way` with automatic conflict-coach hook on failure. Composes with F107 conflict-coach + F109 stash-on-pull.
- [ ] F132: Commit graph PDF export — third format after F61 SVG + F83 PNG; renders the graph via the same webview pipeline but uses a print-style stylesheet + window.print() with `media: print` rules. Higher fidelity than PNG for long graphs.
