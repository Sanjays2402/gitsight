...
- `web/server/index.mjs` — companion. Builders: buildSnapshotForRepo, buildCommitDetailForRepo, buildFileDiffForRepo, **buildBlameForRepo (W12), buildActivityForRepo (W13), buildContributorsForRepo (W14)**, scanReposUnder, resolveRequestRepo, isSafeRev. Routes: /api/{health,repos,graph,commit/:rev,diff,blame,activity,contributors}. Tests: `web/server/index.test.mjs`.
- `web/src/{main,graph,data,theme,themeResolve,palettePicker,format,icons,demo}.ts`, `styles.css`. **main.ts is now a 4-view shell (Graph/Activity/Contributors/Blame) with a tab bar + lazy loaders + W11 responsive drawer.**
- `web/src/{detailPanel,detailFormat}.ts` (W6), `{diffView,diffFormat}.ts` (W7), `repoPicker.ts` (W8), `refRailView.ts` (W9), `{blameView,blameFormat}.ts` (W12), `activityView.ts` (W13), `contributorsView.ts` (W14), `exportGraph.ts` (W15), `responsive.ts` (W11). Tests: `web/src/{themeResolve,detailFormat,diffFormat,responsive,blameFormat}.test.mjs`.
- **Web test-import rule (carry-forward):** node:test files import pure helpers via a relative `.ts` path; never import a module that transitively pulls DOM (`./format`) or the `@shared` alias. That's why W6/W7 split pure helpers (`detailFormat`/`diffFormat`) out of the DOM views.
