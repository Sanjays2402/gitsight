...
- [ ] **W36** Blame reblame-before-commit ("ignore revs") — an input to skip a noise commit (e.g. a mass reformat) so blame attributes the line to its real author; mirrors `git blame --ignore-rev`. Server passes `--ignore-rev`; web adds the control.
...
- [ ] **W41** Graph minimap — a condensed lane overview down the side of the graph for fast scroll-to-region on long histories; reuse the W16 virtual window math. Pure minimap-scale helper + a canvas/SVG strip.
...
### Roadmap refill (W43-W47, added tick 35 — W38/W39/W40/W35/W42 shipped; W36 + W41 still open. Keep deepening the six-view app, keyboard-first + local-first)
- [ ] **W43** Activity year picker — the deferred half of W39: a year selector that re-scopes the calendar to a single calendar year (since/until window on the log), with prev/next arrows. Pure year-window helper in `src/shared/activity.ts`.
- [ ] **W44** Blame ignore-revs (W36 alias) — an input to skip a noise commit (mass reformat) so blame attributes lines to their real author; server passes `git blame --ignore-rev <sha>` (validated), web adds the control + a small chip list of ignored revs.
- [ ] **W45** Graph minimap (W41 alias) — a condensed lane strip down the graph's edge for fast scroll-to-region on long histories; reuse the W16 virtual-window math. Pure minimap-scale helper + an SVG strip + drag-to-scroll.
- [ ] **W46** Diff split-view in the blame + compare deep paths — carry the W38 split toggle into the compare file diffs' default and remember per-surface; plus a "copy as unified/split" affordance. Small wiring slice over the W38 core.
- [ ] **W47** Contributor compare deep-link — a `#contributors?vs=a@x,b@y` hash route (extends W24 hashRoute) so a two-author comparison is shareable; opens the panel pre-loaded. Pure hash grammar + parse, reuse the W35 panel.
