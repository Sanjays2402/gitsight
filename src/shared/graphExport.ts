/**
 * GitSight shared standalone-SVG export (W15).
 *
 * STACK-AGNOSTIC. No `vscode`, no Node, no DOM. Extracted from
 * src/git/commitGraphExport.ts so BOTH the VS Code webview export (F61)
 * and the standalone web app's "Export SVG" share one renderer — the
 * commit-graph export never forks. The vscode/webview-coupled PDF/PNG
 * plumbing stays in commitGraphExport.ts; only the pure SVG document
 * builder + filename helper live here.
 *
 * `src/git/commitGraphExport.ts` now RE-EXPORTS these (mirroring the way
 * `views/graphThemes.ts` re-exports from `shared/graphPalette.ts`), so
 * every existing import site and test keeps working unchanged.
 *
 * Tests: test/git/commitGraphExport.test.ts (via the re-export) +
 *        test/git/graphExport.test.ts (direct).
 */
...
export interface BuildSvgArgs {
  /** The `<g transform="translate(0,N)">…</g>` fragments the surface
   *  already generated for each row. Caller passes them verbatim so the
   *  export visual matches the rendered graph pixel-for-pixel. */
  rowsSvg: string;
  /** Width of the graph portion in pixels. */
  graphWidth: number;
...
export function sanitiseColour(s: string | undefined): string | undefined {
